...
# Parameters
tf.flags.DEFINE_string("hidden_layers_structure", "1000,1000,1000", "Structure of the hidden layers")
tf.flags.DEFINE_float("learning_rate_rbm", 0.01, "Learning rate of the restricted boltzmann machine (RBM)")
tf.flags.DEFINE_float("learning_rate", 0.1, "Learning rate of the deep belief network (ANN)")
tf.flags.DEFINE_integer("n_epochs_rbm", 100, "Number of epochs of the restricted boltzmann machine (RBM)")
tf.flags.DEFINE_integer("n_iter_backprop", 500, "Number of epochs of the deep belief network (ANN)")
...
tf.flags.DEFINE_float("dropout_p", 0, "Dropout rate")
...
for it in range(0,50):
    ...
    for file in range(0,4):
        ...
        if (file == 0):
            filename = "{}{}{}".format('/gs/scratch/cenoi/deep-belief-network/OracleV5C14-training/',it ,'-train-lapd-no.neg')
        # Create the training data (Positive Examples)
        elif (file == 1):
            filename = "{}{}{}".format('/gs/scratch/cenoi/deep-belief-network/OracleV5C14-training/',it ,'-train-lapd-yes.pos')

        # Create the testing data (Negative Examples)
        elif (file == 2):
            filename = "{}{}{}".format('/gs/scratch/cenoi/deep-belief-network/OracleV5C14-training/',it ,'-test-lapd-no.neg')

        # Create the testing data (Positive Examples)
        else:
            filename = "{}{}{}".format('/gs/scratch/cenoi/deep-belief-network/OracleV5C14-training/',it ,'-test-lapd-yes.pos')
